...
export default function AdminInventoryAlertsPage() {
    ...
    const getAlertColor = (type: string) => {
        switch (type) {
            case 'critical':
                return 'bg-red-500/10 text-red-700 dark:text-red-300 border-l-red-500/60'
            case 'out_of_stock':
                return 'bg-orange-500/10 text-orange-700 dark:text-orange-300 border-l-orange-500/60'
            case 'low_stock':
                return 'bg-yellow-500/10 text-yellow-700 dark:text-yellow-300 border-l-yellow-500/60'
            default:
                return 'bg-bg-primary text-text-primary border-l-border-color'
        }
    }
                ...
                <div className="card mb-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-border-color">
                        <div className="text-center">
                            <p className="text-2xl font-bold text-theme-primary">{alerts.length}</p>
                ...
                <div className="space-y-4">
                    {alerts.length === 0 ? (
                        ...
                        alerts.map((alert) => (
                            <div
                                key={alert.id}
                                className={`rounded-lg shadow-sm p-6 border border-border-color border-l-4 ${getAlertColor(alert.alert_type)}`}
                            >
                                <div className="flex items-start justify-between">
                                    <div className="flex items-start gap-4 flex-1">
                                        <div className="mt-1 text-current">{getAlertIcon(alert.alert_type)}</div>
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-current mb-3">{alert.message}</p>
                                    ...
                                    <div className="flex gap-2 ml-4">
                                        {!alert.is_resolved && (
                                            <button
                                                ...
                                                className="btn bg-green-500/10 text-green-700 dark:text-green-300 hover:bg-green-500/20"
                                            >
                                                <CheckCircle size={20} />
                                            </button>
                                        )}
                ...
                {showCreateModal && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                        <div className="bg-bg-primary border border-border-color rounded-lg max-w-2xl w-full p-6">
                            <h2 className="text-2xl font-bold text-text-primary mb-4">Create Inventory Alert</h2>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-text-primary mb-2">Current Quantity</label>
                                        <input
                                            type="number"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-text-primary mb-2">Threshold Quantity</label>
                                        <input
                                            type="number"
